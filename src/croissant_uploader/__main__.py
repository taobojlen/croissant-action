...
def main():
    parser = argparse.ArgumentParser(
        description="Upload test results to Croissant"
    )
    parser.add_argument(
        "--hostname",
        required=True,
        help="The hostname of the Croissant server, e.g. croissant.example.workers.dev",
    )
    parser.add_argument(
        "--branch", required=True, help="The branch name that the tests ran on"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
    ...
    parser.add_argument(
        "files", nargs="*", help="The path to the test results"
    )
