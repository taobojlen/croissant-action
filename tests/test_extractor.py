...
def test_handles_test_in_multiple_pytest_files():
    ...
    assert results.passed == {"tests.test_foo.test_foo", "tests.test_bar.test_bar"}
    assert results.failed == set()
...
def handles_duplicate_tests_last_one_wins():
    results = extract_tests("tests/test_data/fooskipped.xml", "tests/test_data/foosuccess.xml", "tests/test_data/foofailure.xml")
    assert results.passed == set()
    assert results.failed == {"tests.test_extractor.test_foo"}

    results = extract_tests("tests/test_data/foofailure.xml", "tests/test_data/fooskipped.xml", "tests/test_data/foosuccess.xml")
    assert results.passed == {"tests.test_extractor.test_foo"}
    assert results.failed == set()

    results = extract_tests("tests/test_data/foofailure.xml", "tests/test_data/foosuccess.xml", "tests/test_data/fooskipped.xml")
    assert results.passed == set()
    assert results.failed == set()